...
# The directories the login-shell files install into. All of them are present on the image, so the
...
# The bash-completion dispatcher, which is what install_completions guards on -- the debian:*-slim
# images have neither it nor the package. A stub rather than `apt-get install bash-completion`:
# the installer only tests the path, and a package install here would cost a network round trip
# in every board-test run.
mkdir -p /usr/share/bash-completion
touch /usr/share/bash-completion/bash_completion

# An eager copy from a release that predates the move, so the migration below is asserted against
# a board in the state the whole fleet is in rather than a clean one.
cat > /etc/bash_completion.d/robotctl <<"OLDCOMPAT"
...
test -f /usr/share/bash-completion/completions/robotctl \
    || { echo "    [FAIL] the robotctl completions were not installed"; exit 1; }

# And the eager copy gone. Leaving it would shadow the file above -- /etc/bash_completion.d is
# sourced in full at every login, so the loader there would register first and the move would
# buy nothing on any board that had ever run an older release.
test -f /etc/bash_completion.d/robotctl \
    && { echo "    [FAIL] the eager copy in /etc/bash_completion.d survived, and shadows the new one"; exit 1; }
...
for f in /etc/profile.d/robot-name-prompt.sh /usr/share/bash-completion/completions/robotctl \
    /etc/update-motd.d/40-robot; do
    test -f "$f" \
