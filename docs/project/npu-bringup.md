...
Fill in from a real board. What matters:

| | measured | notes |
|---|---|---|
| driver / runtime | | `setup-npu.sh` prints both |
| latency p50 / p95 | | inference plus decode, not JPEG decoding |
| cpu per frame | | the NPU should leave this small |
